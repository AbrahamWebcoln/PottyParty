...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
        ...
        if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
            ...
            $link = '';
        $calendar_output .= '<th class="month_cur" colspan="7">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';
        ...
        if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
            ...
            $link = '';
        $calendar_output .= '<th class="month_cur" colspan="5">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth-1), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';
...
if (!function_exists('themerex_get_post_data')) {
	function themerex_get_post_data(&$opt, $post_obj=null) {
		...
		if (!empty($opt['date_format'])) {
			...
			$post_date_part1 = empty($parts[0]) ? '' : date_i18n($parts[0], $post_date_stamp);
			$post_date_part2 = empty($parts[1]) ? '' : date_i18n($parts[1], $post_date_stamp);
