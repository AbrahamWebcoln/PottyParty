...
class themerex_dummy_data_importer {
	...
	var $options = array(
		...
		'demo_url'				=> 'http://education.themerex.net/',	// URL of the demo site - need for change URLs in the custom menu items
	...
	function checkRequiredPlugins() {
        ...
        if (in_array ('additional_tags', $this->options['required_plugins']) && !themerex_exists_additional_tags())
            $not_installed .= '<br>Additional Tags';
		$not_installed = apply_filters('themerex_filter_importer_required_plugins', $not_installed, $this);
		if ($not_installed) {
