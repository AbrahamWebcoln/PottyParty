...
// Add Product author
if ( ! function_exists( 'themerex_woocommerce_template_loop_author' ) ) {
    /**
     * Display the average author in the loop
     *
     * @subpackage	Loop
     */
    function themerex_woocommerce_template_loop_author() {
        ?>
...
if ( !function_exists( 'themerex_woocommerce_product_cats' ) ) {
    function themerex_woocommerce_product_cats()
    {
        ...
        ?><a class="hover_featured_link sc_button sc_button_square sc_button_style_filled sc_button_bg_link sc_button_size_small" href="<?php echo get_permalink(); ?>"><?php echo esc_html(__('View', 'themerex'));?></a><?php
        echo '</div>';
    }
}
add_action('woocommerce_before_shop_loop_item_title', 'themerex_woocommerce_product_cats');

// Add wrapper to title,price of features products
if ( ! function_exists( 'themerex_woocommerce_open_featured_products_wrapper' ) ) {
    function themerex_woocommerce_open_featured_products_wrapper() {
        // Show product categories
        ?><div class="featured-product-wrapper"><?php
    }
}
if ( ! function_exists( 'themerex_woocommerce_close_featured_products_wrapper' ) ) {
    function themerex_woocommerce_close_featured_products_wrapper() {
        ?></div><?php
    }
}
add_action(    'woocommerce_before_shop_loop_item_title',	'themerex_woocommerce_open_featured_products_wrapper', 15 );
add_action(    'woocommerce_after_shop_loop_item_title',	'themerex_woocommerce_close_featured_products_wrapper', 25 );


// Add wrapper to features products
if ( ! function_exists( 'themerex_woocommerce_open_featured_products_wrap' ) ) {
    function themerex_woocommerce_open_featured_products_wrap() {
        ?><div class="featured-product-wrap"><?php
    }
}
if ( ! function_exists( 'themerex_woocommerce_close_featured_products_wrap' ) ) {
    function themerex_woocommerce_close_featured_products_wrap() {
        ?></div><?php
    }
}
add_action(    'woocommerce_before_shop_loop_item',	'themerex_woocommerce_open_featured_products_wrap', 10 );
add_action(    'woocommerce_after_shop_loop_item',	'themerex_woocommerce_close_featured_products_wrap', 15 );



// Add wrapper to features image
if ( ! function_exists( 'themerex_woocommerce_open_featured_product_wrap_image' ) ) {
    function themerex_woocommerce_open_featured_product_wrap_image() {
        ?><div class="featured-image"><?php
    }
}
if ( ! function_exists( 'themerex_woocommerce_close_featured_product_wrap_image' ) ) {
    function themerex_woocommerce_close_featured_product_wrap_image() {
        ?></div><!-- .featured-image --><?php
    }
}
add_action(    'woocommerce_before_shop_loop_item',	'themerex_woocommerce_open_featured_product_wrap_image', 15 );
add_action(    'woocommerce_before_shop_loop_item_title',	'themerex_woocommerce_close_featured_product_wrap_image', 10 );




add_action('woocommerce_after_shop_loop_item_title', 'themerex_woocommerce_template_loop_author', 3);
...
// Add taxonomy author & publisher
if ( !function_exists( 'themerex_woocommerce_show_product_author_publisher' ) ) {
    add_action('woocommerce_product_meta_end', 'themerex_woocommerce_show_product_author_publisher', 5);
    function themerex_woocommerce_show_product_author_publisher() { ?>
        <div class="data-top">
            <div class="product-author">
                <span>
                    <?php
                    global $post, $publishers, $authors;
                    $authors = wp_get_object_terms($post->ID, 'authors');
                    if (!$authors==0) {echo esc_attr(__('Author: ', 'themerex'));};?>
            ...
            <div class="product_meta">
                <span class="posted_in">
                    <?php
                    $publishers = wp_get_object_terms($post->ID, 'publisher');
                    if (!$publishers==0) {echo esc_attr(__('Publisher: ', 'themerex'));};?>
                    <?php foreach ($publishers as $publisher) { ?>
                        <a href="<?php echo get_term_link($publisher->slug, 'publisher'); ?>"><?php echo esc_html($publisher->name) . ' ' ; ?></a>
                    <?php } ?>
                </span>
            </div>
        </div>
...
if ( !function_exists( 'themerex_woocommerce_before_shop_loop' ) ) {
	...
	function themerex_woocommerce_before_shop_loop() {
		...
		if (themerex_get_custom_option('show_mode_buttons')=='yes') {
			echo '<div class="mode_buttons"><form action="' . esc_url(themerex_get_protocol().'://' . ($_SERVER["HTTP_HOST"]) . ($_SERVER["REQUEST_URI"])).'" method="post">'
...
// Show product tag
if ( !function_exists( 'themerex_woocommerce_product_loop_tags' ) ) {
//add_action('woocommerce_after_shop_loop_item_title', 'themerex_woocommerce_product_loop_tags');
    function themerex_woocommerce_product_loop_tags()
    {
        global $post, $product;
