		...
		if (!$post_data['post_protected'] && (
			...
			if (!empty($post_options['dedicated'])) {
				...
				if ($title_over) {
					...
					<div class="post_thumb_hover"<?php echo esc_attr($header_style); ?>>
						...
						<?php echo '<div class="announcement-arrow"></div>';?>
					</div>
					<?php
				}
			}
			...
			if (($countdown_style = themerex_get_custom_option('show_countdown')) > 0 && $course_start > date('Y-m-d')) {
				...
				<div class="post_countdown">
					<h4 class="post_countdown_title"><?php echo ($parent_id ? __('This Lesson Starts In', 'themerex') : __('Sales Start In', 'themerex')); ?></h4>
				...
				<div class="post_access_denied">
					...
					echo '<p>'
						. ($user_bought_course
							...
							: __('Unfortunately, you are not permitted to view this announcement!', 'themerex')
							)
						. '</p>';
					if (!$user_bought_course && $product_link != '') {
						echo '<p>'. __('To purchase it - click the button below:', 'themerex'). '</p>';
						echo '<div class="post_button">'.do_shortcode('[trx_button size="medium" link="'.esc_url($product_link).'"]'.__('Apply to Announcement Now', 'themerex').'[/trx_button]').'</div>';
					}
					?>
				</div>
				<?php
